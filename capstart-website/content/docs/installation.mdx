  ...
  <div className="flex flex-col gap-4 rounded-lg border border-fd-border bg-fd-card p-5">
    <div className="flex flex-col gap-2">
      ...
      <p className="text-sm leading-relaxed text-fd-muted-foreground">
        Start from a product-ready React app with Supabase auth, Tailwind v4, shadcn/ui, protected routes, mobile layout, and Capstart CLI native generation.
      </p>
    </div>
...
Use Capstart when you want a product app baseline immediately: Supabase session handling, protected routes, shadcn/ui components, mobile layout, and Capacitor-ready native integrations. The boilerplate does not commit generated native projects; run the CLI once so `ios/` and `android/` are created with your real bundle id and app name.
...
After cloning, fill the Supabase values in `.env`, then choose your final `--app-id` and `--app-name` before running Capstart. The generated `ios/` and `android/` folders belong to your app and should be committed.
