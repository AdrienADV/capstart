...
Use Capstart when you want a product app baseline immediately: Supabase session handling, protected routes, shadcn/ui components, and native iOS/Android projects already wired together. The CLI can create this boilerplate directly, so you do not need to run `degit` yourself.
...
After creating the app, fill the Supabase values in `.env`, then replace icons,
splash screens, signing, and store metadata before shipping.
