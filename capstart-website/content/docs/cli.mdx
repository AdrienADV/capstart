...
Capstart CLI turns an existing **Next.js**, **Nuxt**, standalone **React + Vite**, **Svelte + Vite**, **SvelteKit**, **TanStack Start**, or standalone **Vue** project into a Capacitor app. It detects the framework and package manager, configures the native app id/name, configures a client-side production build when needed, installs Capacitor, creates the native projects, and adds scripts for the day-to-day mobile workflow.

Use the CLI when you already have a web application and want to keep its existing structure. For a new React app with auth, protected routes, and mobile UI conventions already wired, start from the [Capstart boilerplate](/docs/installation#capstart-boilerplate), then run this CLI to generate the native projects with your app id.
...
Before changing files, Capstart confirms the detected framework and asks for the native app name, native app id, setup profile, and safe-area behavior. It then runs the complete setup:
...
| Capacitor config | Creates or updates `capacitor.config.*`, including `appId`, `appName`, and the detected `webDir` |
